...
const BuyEnergy = () => {
    ...
    async function buy(listingId, amount, priceWei) {
        try {
            if (!address) await connectWallet();

            const totalCost = BigInt(amount) * BigInt(priceWei); 

            const tx = await writeContract(client, {
                address: ET.MARKETPLACE_ADDRESS,
                abi: ET.EnergyMarketplaceABI,
                functionName: "buyEnergy",
                args: [Number(listingId), BigInt(amount)],
                account: address,
                value: totalCost, 
            });
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 border border-green-200">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button 
                            ...
                            className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl transition-all duration-200 flex items-center space-x-2"
                        >
                    ...
                    {listings.length === 0 && !loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {listings.map((l) => (
                                <div key={l.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-all duration-200 bg-white">
                                    <div className="flex items-start justify-between mb-4">
                                        <div className="flex-1">
                                            <h4 className="font-semibold text-gray-800 mb-2">Energy Listing #{l.id + 1}</h4>
                                            <div className="space-y-2 text-sm">
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-gray-500">Producer:</span>
                                                    <span className="font-mono text-xs bg-gray-100 px-2 py-1 rounded">
                                                        {l.producer.slice(0, 6)}...{l.producer.slice(-4)}
                                                    </span>
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-gray-500">Amount:</span>
                                                    <span className="font-semibold text-green-600">{l.amount} ENG</span>
                                                </div>
                                                <div className="flex items-center space-x-2">
                                                    <span className="text-gray-500">Price:</span>
                                                    <span className="font-semibold text-emerald-600">{l.priceEth} ETH</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <button
                                        onClick={() => buy(l.id, l.amount, l.priceWei)}
                    ...
                    {txHash && (
                        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-xl">
                            <p className="text-sm font-medium text-green-800 mb-2">Purchase Successful!</p>
